...
function ItemDAO(database) {
    ...
    this.getCategories = function (callback) {
        ...
        this.db.collection('item').aggregate([
            ...
            result.push({
                _id: "All",
                num: result.reduce((previousValue, currentValue) => previousValue + currentValue.num, 0)
            });
            result.sort((a, b) => a._id.toLowerCase() > b._id.toLowerCase());
            callback(result);
        });
    };
    ...
    this.getItems = function (category, page, itemsPerPage, callback) {
        ...
        this.db.collection('item').find(query, options)
            .toArray((err, result) => {
                assert.equal(err, null);
                callback(result);
            });
    ...
    this.searchItems = function (query, page, itemsPerPage, callback) {
        ...
        this.db.collection('item').find(myQuery, options)
            .toArray((err, result) => {
                assert.equal(err, null);
                callback(result);
            });
    ...
    this.getNumSearchItems = function (query, callback) {
        ...
        this.db.collection('item').find(myQuery)
            .count((err, count) => {
                assert.equal(err, null);
                callback(count);
            });
